...
public class MaximumProductofSplittedBinaryTree {
    ...
    private void dfsSum(TreeNode root) {
        ...
        int treeSum = 0;
        if (root.left != null) {
            this.dfsSum(root.left);

            treeSum = (treeSum + root.left.val);
        }
        if (root.right != null) {
            this.dfsSum(root.right);

            treeSum = (treeSum + root.right.val);
        }
    ...
    // TODO: implement using dfs and compare performance
    private long findMaxSumBfs(TreeNode root) {
        long maxProductOfSums = 0;
        ...
        while (!bfs.isEmpty()) {
            ...
            if (head.left != null) {
                ...
                product = (long)head.left.val * (long)(root.val - head.left.val);
                if (product > maxProductOfSums) {
                    maxProductOfSums = product;
                }
            }
            if (head.right != null) {
                bfs.offer(head.right);

                product = (long)head.right.val * (long)(root.val - head.right.val);
                if (product > maxProductOfSums) {
                    maxProductOfSums = product;
                }
            }
